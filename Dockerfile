...
RUN apt-get update && \
    apt-get install --no-install-recommends -y naemon-core naemon-livestatus monitoring-plugins monitoring-plugins-contrib nagios-nrpe-plugin file jq eapoltest && \
    rm -rf /var/lib/apt/lists/*
